...
\usepackage[margin=1.2cm,columnsep=6mm]{geometry}
\usepackage{graphicx}
\usepackage{parskip}
\usepackage{enumitem}
\usepackage{xcolor}
...
\textbf{Young-féle optimális SOR-tétel (egy gyakori speciális alak).} \quad
{\normalfont\small SOR = \textit{Successive Over-Relaxation} (egymást követő túlrelaxáció)}

\medskip
\textit{Feltételek:}
\begin{enumerate}[noitemsep,topsep=2pt]
  \item $A$ szimmetrikus és pozitív definit;
  \item $A$ tridiagonális szerkezetű;
  \item a Jacobi-iteráció $B_J$ mátrixának sajátértékei valósak.
\end{enumerate}

\medskip
\textit{Állítás:} Ha $\varrho(B_J)<1$, akkor az optimális relaxációs paraméter
\[
  \omega_0 = \frac{2}{1+\sqrt{1-\varrho(B_J)^2}},
\]
és a hozzá tartozó SOR-iteráció spektrálsugara
\[
  \varrho(B_{\omega_0}) = \omega_0 - 1.
\]
Továbbá minden $\omega\in(0,2)$ esetén $\varrho(B_\omega)<1$, tehát az SOR-iteráció konvergens, és
\[
  \varrho(B_{\omega_0}) \le \varrho(B_\omega),
\]
vagyis $\omega_0$ valóban optimális.
\end{mdframed}
...
(c) $A_3 = \scalebox{0.78}{$\begin{bmatrix}4&-1&-1&0\\-1&4&0&-1\\-1&0&4&-1\\0&-1&-1&4\end{bmatrix}$}$
...
\begin{gathered}
A = \begin{bmatrix}5&1\\1&5\end{bmatrix},\quad b = \begin{bmatrix}6\\6\end{bmatrix},\\
x^{(0)} = \begin{bmatrix}0\\0\end{bmatrix},\quad p = \tfrac{1}{5}
\end{gathered}
\]
...
$$f(x) = x^3 - 2x - 5, \quad \textrm{intervallum} [2, 3]$$.

Végezz \textbf{3 intervallumfelezési lépést} kézzel az $f(x) = 0$ egyenlet megoldására! Töltsd ki a táblázatot:

\begin{center}
\begin{tabular}{c|c|c|c|c|c}
\hline
$k$ & $a$ & $b$ & $c=\frac{a+b}{2}$ & $f(c)$ & új int.\\
\hline
0 & 2 & 3 & 2.5 &  & [2,2.5] \\
1 & 2.5 & 3 & \ldots & \ldots & \ldots\\
